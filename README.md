...
- In-memory and optional disk caching of normalized docs
 - Integrated preview using browser via `marked`
...
require('aspire_docs').setup({
  ...
  preview_renderer = "browser", -- "browser" | "none"
...
Notes about dependencies and preview
 ...
 - The plugin is usable without previews. If you prefer no preview set `preview_renderer = "none"`.

CheckHealth behavior
- Currently the plugin does not register a CheckHealth provider. That means `:checkhealth` will not report missing external preview binaries automatically.
